...
$(document).on("click", "#btnSendPhone", function (e) {
  ...
  if (ctype) {
    ...
    try {
      ...
      Sentry.captureMessage("Telefone inválido.")
...
$(document).on("click", "#btnValidateSms", function (e) {
  ...
  if (!ctype) {
    $.ajax({
      ...
      success: function (e) {
        if (e.status_code == 200) {
          if(e.is_email_updated) {
            toastr.success(e.message);
            window.location.replace("/vendorUsers/");
          }
          else {
            toastr.warning("O email informado já está associado a uma conta.");
            window.location.replace("/vendorUsers/"+ e.usershopee_id);
          }
        }
        if (e.status_code == 404) {
...
$(document).on("click", "#btnValidateUserAli", function () {
  ...
  $.ajax({
    type: "POST",
    url: "/aliUserValidateJson/",
    headers: { "X-CSRFToken": csrftoken },
    data: $("#userAliForm").serialize(),
    success: function (response) {
      var responseData = response;
      if("error" in responseData) {
        Sentry.captureMessage(responseData.error);
        toastr.error(responseData.error);
      }

      if("success" in responseData) {
        toastr.success(responseData.success);
        window.location.href = "/vendorUsers/";
      }
